  ...
  try {
    ...
    if (e.message === "GROQ_KEY_MISSING" && !localAvailable) {
      throw new Error(
        ...
        "Please contact the administrator to set up the Groq API key."
      );
    }
    throw e;
  }
...
export async function getAnalysisMethod() {
  ...
  if (local) return { method: "hybrid", label: "Local AI + Groq Cloud", icon: "🧬" };
  return { method: "groq", label: "Groq Vision AI (LLaMA)", icon: "⚡" };
}
